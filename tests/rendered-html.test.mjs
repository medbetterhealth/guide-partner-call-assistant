...
test("five outreach pathways use distinct email logic and no patient/client claim", async () => {
  ...
  assert.match(submitRoute, /options: optionLabels/);
  assert.doesNotMatch(submitRoute, /optionLabels\.map/);
...
test("pipeline UI keeps readable horizontally scrolling Kanban cards", async () => {
  ...
  assert.match(html, /\.kanban-col\{[\s\S]*?flex:0 0 318px;[\s\S]*?min-width:318px;/);
  ...
  assert.match(html, /class="quick-delete deal-delete-btn"/);
  assert.match(html, /id="pipelineFilterToggle"/);
  assert.match(html, /id="pipelineSortToggle"/);
  assert.match(html, /class="pipeline-legend"/);
  assert.match(html, /All times shown in EST/);
