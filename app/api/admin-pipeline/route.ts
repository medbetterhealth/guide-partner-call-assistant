...
const DESIRED_STAGES = [
  ...
  {
    name: "Gatekeeper Only – No Decision Maker Information",
    sources: ["Gatekeeper Only – No Decision Maker Information", "Outreach Made", "Follow-Up Needed"],
  },
  {
    name: "Decision Maker Identified – Email Provided",
    sources: ["Decision Maker Identified – Email Provided", "Decision Maker Identified – Email Sent", "Email Sent"],
  },
  {
    name: "Decision Maker Reached – Appointment Not Scheduled",
    sources: ["Decision Maker Reached – Appointment Not Scheduled", "Decision Maker Contacted", "Decision Maker Contacted – Email Sent"],
  },
  {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Never silently delete a legacy stage that still contains opportunities.
    // This matters in particular when both Outreach Made and Follow-Up Needed
    // already exist. Move those records to the intended new outcome first.
    const retainedStageIds = new Set(desired.flatMap((item) => item.source?.id ? [item.source.id] : []));
    const occupiedObsoleteStages = currentStages
      .filter((stage) => !retainedStageIds.has(stage.id))
      .map((stage) => ({
        stage,
        count: opportunitiesBefore.filter((opportunity) => opportunity.pipelineStageId === stage.id).length,
      }))
      .filter((item) => item.count > 0);
    if (occupiedObsoleteStages.length) {
      throw new Error(`Cannot safely remove legacy stages with active opportunities: ${occupiedObsoleteStages.map((item) => `${item.stage.name} (${item.count})`).join(", ")}. Move those opportunities first, then run the migration again.`);
    }
