...
export async function POST(request: Request) {
  try {
    ...
    const initialEmailStatus = path.shouldEmail ? "Pending automation" : "Not sent – follow-up needed";
    const customFields = [
    ...
    const stageCandidates = path.key === "gatekeeper_transferred" || path.key === "answerer_is_decision_maker"
      ? ["Decision Maker Contacted", "Decision Maker Contacted – Email Sent"]
      : path.key === "follow_up_needed"
        ? ["Follow-Up Needed", "Outreach Made"]
        : [path.stageName];
    const stage = stageCandidates.map((name) => pipeline.stages.find((item) => item.name === name)).find(Boolean) || pipeline.stages[0];
